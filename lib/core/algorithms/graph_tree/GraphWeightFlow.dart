  ...
  void generate_graph() 
  {
    ...
    for(var paths in neighbours.entries) 
    {
      for(var vertex in paths.value)
      {
        ...
        bool fl_len = (vertex_summ[vertex] ?? 0) < 3 && (vertex_summ[paths.key] ?? 0) < 3;
  ...
  void check_fill()
  {
    ...
    for(var pair in vertex_to.entries)
    {
      if(pair.value==0) //если в эту вершину нет путей
      {
        ...
        LinkedHashMap<int, int> sortedMap = LinkedHashMap.fromEntries(sortedEntries);
        print(sortedMap);
      ...
      if(pair.value==vertex_summ[pair.key]) //если в эту вершину есть пути, но из неё нет
      {
        var sortedEntries = vertex_summ.entries.toList()
        ..sort((a, b) => a.value.compareTo(b.value));
