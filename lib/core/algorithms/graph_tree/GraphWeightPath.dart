...
class GraphWeightPath extends GraphWeight
{
  ...
  void generate_graph() 
  {
    ...
    MapEntry<int, int> min_to = find_min(vertexes_to); //вершина из которой пойдём
    MapEntry<int, int> min_in = find_min(vertexes_in); //вершина в которую пойдём

    while((min_to.value < 3 || min_in.value<3) && fl)
    {
      bool was_reverse = (w_graph[min_in.key]?.containsKey(min_to.key) ?? false); //есть уже обратный путь
      bool was_yet = (w_graph[min_to.key]?.containsKey(min_in.key) ?? false); //уже существует путь, который мы хотим построить

      if(min_to.key!=min_in.key && !(was_yet||was_reverse)) //если вершины не совпадают и два условия выше выполняются
      {
      ...
      else
      {
        ...
        LinkedHashMap<int, int> sorted_in = LinkedHashMap.fromEntries(sortedEntries_in);
        ...
        for(var ver_in in sorted_in.entries)
        {
          ...
          if(min_to.key == ver_in.key || (was_yet||was_reverse)) continue;
          else
          {
            w_graph[min_to.key]?[ver_in.key] = random.nextInt(5)+3;
            vertexes_to[min_to.key] = (vertexes_to[min_to.key] ?? 0) + 1;
            vertexes_in[ver_in.key] = (vertexes_in[ver_in.key] ?? 0) + 1;
            fl = true; //мы не перебрали все варианты -> нашли подходящий -> можно продолжать цикл
            break;
          }
        }
      }
