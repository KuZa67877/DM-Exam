...
class TreePainter extends CustomPainter {
  ...
  void _calculateNodePositions(Node node, double x, double y, double offsetX,
    ...
    if (node.childs.isNotEmpty) {
      double stepX = offsetX / node.childs.length;
      double startX = x - (node.childs.length * stepX / 2) + stepX / 2;

      // Для корневого узла используем большее смещение
      if (node == treeRoot) {
        startX = x - (node.childs.length * stepX);
        stepX *= 2;
      }

      for (int i = 0; i < node.childs.length; i++) {
        if (node.childs[i] != null) {
          double childX = startX + i * stepX;
          double childY = y + offsetY;

          // Проверка на накладку координат и корректировка по вертикали
          for (var position in positions.values) {
            if ((position - Offset(childX, childY)).distance < offsetY) {
              childY += offsetY / 2;
            }
          }

          _calculateNodePositions(
              node.childs[i]!, childX, childY, offsetX / 2, positions, offsetY);
        }
      }
    }
  }
